...
trait Auth0SecuredApi extends Auth0SecurityComponent {
  ...
  implicit def request2profile[A](req: Auth0AuthenticatedRequest[A]): Auth0SecurityUser = {
    req.user
  }

  case class Auth0AuthenticatedRequest[A](user: Auth0SecurityUser, request: Request[A]) extends WrappedRequest[A](request)
  ...
  case object Auth0SecuredAction extends ActionRefiner[Request, Auth0AuthenticatedRequest] {
    override protected def refine[A](request: Request[A]): Future[Either[Result, Auth0AuthenticatedRequest[A]]] = {
      request.headers.get("Authorization").fold {
        ServiceResult.failedF[Auth0SecurityUser](InvalidRequestError(Seq("`Authorization` header not found")))
      } { header =>
        val bearerToken: String = "Bearer "
        if (!header.startsWith(bearerToken)) {
          ServiceResult.failedF[Auth0SecurityUser](InvalidRequestError(Seq(s"Bad Authorization header, don't start by `$bearerToken`")))
        } else {
